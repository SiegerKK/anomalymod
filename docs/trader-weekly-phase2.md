...
Review `quantity`, `probability`, `original_tiers`, `effective_section`, and `category`, then propose explicit per-item quantity/probability changes. Do not auto-scale all tiers to a seven-day restock period and do not change balance before the table is reviewed.
